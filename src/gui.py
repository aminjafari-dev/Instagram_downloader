...
class DownloaderGUI:
    ...
    def _build_ui(self) -> None:
        ...
        out_entry.grid(row=3, column=1, sticky="ew", **pad)
        ttk.Button(frm, text="Browse", command=self._browse_output).grid(row=3, column=2, sticky="ew", **pad)

        # Cookies file
        ttk.Label(frm, text="Cookies file (optional):").grid(row=4, column=0, sticky="w", **pad)
        cookies_entry = ttk.Entry(frm, textvariable=self.cookies_var, width=50)
        cookies_entry.grid(row=4, column=1, sticky="ew", **pad)
        ttk.Button(frm, text="Browse", command=self._browse_cookies).grid(row=4, column=2, sticky="ew", **pad)
        ...
        self.download_btn.grid(row=7, column=0, columnspan=3, sticky="ew", **pad)

        for i in range(3):
            frm.columnconfigure(i, weight=1)

        # Bind checkbox to show/hide max videos
    ...
    def _toggle_page_mode(self) -> None:
        ...
        if self.page_mode_var.get():
            self.max_videos_label.grid(row=2, column=0, sticky="w", padx=8, pady=6)
            self.max_videos_spinbox.grid(row=2, column=1, sticky="w", padx=8, pady=6)
        else:
